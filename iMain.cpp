...
void drawHawk() {
	if (hawk.visible) {
		if (hawk.direction == 0){
			...
			iShowBMP2(hawk.x, hawk.y, "Images\\Hawk 3.bmp", 0xFFFFFF);
		}
		if (hawk.direction == 1){
			iShowBMP2(hawk.x, hawk.y, "Images\\Hawk 2.bmp", 0xFFFFFF);
		}
	}
}
void drawSquirrel() {
...
void DrawStartPage() {
	...
	/*if (isMouseOver(396, 215, 607, 299))
		...
		iShowBMP(400, 215, "Images\\Button1.bmp");*/  //for level 3 button image

	iText(440, 447, "LEVEL - 1", GLUT_BITMAP_TIMES_ROMAN_24);
	iText(440, 347, "LEVEL - 2", GLUT_BITMAP_TIMES_ROMAN_24);
	//iText(440, 247, "LEVEL - 3", GLUT_BITMAP_TIMES_ROMAN_24); //for level 3 text image
